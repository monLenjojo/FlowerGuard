...
//        LoginActivity.this.finish();
     ...
     */
    private void attemptLogin() {//登入嘗試，判斷是否有錯誤
        if (mAuthTask != null) {
            return;
        }
        // Reset errors.錯誤訊息重新設定
        ...
        if (TextUtils.isEmpty(email)) {//使用TextUtils類別判斷email字串使否為空
            mEmailView.setError(getString(R.string.error_field_required));
            focusView = mEmailView;
            cancel = true;
        } else if (!isEmailValid(email)) {//使用EmailValid副程式判別email字串使否含有@
            mEmailView.setError(getString(R.string.error_invalid_email));
            focusView = mEmailView;
            cancel = true;
        }

        if (cancel) {//經過上面If判斷後設定cancel變數，若為真
            // There was an error; don't attempt login and focus the first
            // form field with an error.
            focusView.requestFocus();//將畫面鎖定在該物件上
        } else {
